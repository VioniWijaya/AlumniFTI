...
exports.updateEventStatus = async (req, res) => {
    try {
        const { alumniId, pengajuanEventId } = req.params;
        const { status, reason } = req.body; // Ambil status dan alasan dari request body
        ...
        // Validasi untuk alasan penolakan
        if (status === 'Pengajuan Ditolak' && (!reason || reason.trim() === '')) {
            return res.status(400).json({ error: 'Alasan penolakan wajib diisi jika status adalah Pengajuan Ditolak.' });
        }

        // Update status dan alasan jika diperlukan
        event.status = status;
        if (status === 'Pengajuan Ditolak') {
            event.deskripsi = reason; // Pastikan kolom ini ada di model Anda
        } else {
            event.deskripsi = null; // Reset alasan jika status bukan "Ditolak"
        }
